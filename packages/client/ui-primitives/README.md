...
| `icons/*`, `BrandMark`, `BrandWordmark`, `ReferenceIcon`, `LinkIcon` | Glyphs and brand marks. Use `LinkIcon` for 14px clickable-link categories. |
...
- **Glyph-level icons are redrawn approximations** — the sparkle mark comes from a font glyph whose vector geometry is not exportable from the local design data; a hand-authored recreation stands in until an exact export path exists.
